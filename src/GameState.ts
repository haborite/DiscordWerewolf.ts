...
export default class GameState {
    ...
    async tryPreparingGame(message : Discord.Message){
        const current_num = Object.keys(this.members).length;
        if(current_num < this.reqMemberNum){
    ...
    cutTimeCheck(interaction : Discord.ButtonInteraction) {
        ...
        if (!isAdd) {
            ...
            this.channels.Living.send(txt);
            if (now >= req) {
                this.channels.Living.send(format(this.langTxt.p4.cut_time_approved, this.ruleSetting.day.reduction_time));
